...
class SlurmDirectives:
    ...
    def fmt_dirs(self) -> str:
        bool_dirs = [f"#SBATCH {f.metadata.get('syntax')}"
                    for f in fields(self) if self.__dir_val(f.name)
                    and isinstance(self.__dir_val(f.name), bool)]
        dirs      = [f"#SBATCH {f.metadata.get('syntax')} {self.__dir_val(f.name)}"
                    for f in fields(self) if self.__dir_val(f.name) is not None
                    and not isinstance(self.__dir_val(f.name), bool)]
        return "\n".join(dirs+bool_dirs)

    def __str__(self):
        return "\n".join(
            [f"{f.metadata.get('syntax')} {self.__dir_val(f.name)}" for f in fields(self)]
        )
...
class SlurmScript(BashScript):
    ...
    def _generate_script(self) -> None:
        content = "\n".join(["#!/bin/bash",self.dirs.fmt_dirs(),
            "",self.BEGIN_SCRIPT,"",
            self._cmds_str(),"",
            self.TRAILER_SCRIPT,""
        ])
