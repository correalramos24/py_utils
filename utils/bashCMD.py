...
class BashCmd(MetaAbstractClass):
    ...
    def run(self, cmd: str) -> "BashCmd":
        """Internal executor"""
        full_cmd = " ".join([cmd] + self._args)
        out_trg = subprocess.PIPE if self._log_file is None else None
        err_trg = subprocess.STDOUT
        if self._log_file:
            with open(self._log_file, "w") as fd:
                r = subprocess.run(full_cmd, cwd=self.p, shell=True,text=True,stdout=fd, stderr=err_trg)
        else:
            r = subprocess.run(full_cmd, cwd=self.p, shell=True, text=True,stdout=out_trg, stderr=err_trg)
    ...
    def ret_code(self) -> Optional[int]: return None if self._cmplt_run is None else self._cmplt_run.returncode
